...
function getLocalStoragePreset() {
  if (typeof window !== 'undefined') {
    const localStorageValue = localStorage.getItem('raahetijarat-preset');
...
const colorPresetsAtomWithPersistence = atom(
  ...
  (get, set, newStorage) => {
    ...
    localStorage.setItem('raahetijarat-preset', JSON.stringify(newStorage));
  }
);
...
const colorPresetNameAtom = atom(
  typeof window !== 'undefined'
    ? localStorage.getItem('raahetijarat-preset-name')
    : DEFAULT_PRESET_COLOR_NAME
);
...
const colorPresetNameAtomWithPersistence = atom(
  ...
  (get, set, newStorage) => {
    ...
    localStorage.setItem('raahetijarat-preset-name', newStorage as string);
  }
);
